 ...
 *
 * @export
 * @class V1BaseAppInitService
 * @typedef {V1BaseAppInitService}
 */
@Injectable({ providedIn: 'root' })
export class V1BaseAppInitService {
  /* General //////////////////////////////////////////////////////////////// */
  ...
  protected readonly _assetsFolder: string = 'assets';

  /** Whether to load data (Build) JSON file or not (set to true, if app already has such file). */
  protected readonly _loadDataBuild: boolean = true;
  ...
  init(environment: any) {
    ...
    const loadDataBuild = (shallLoad: boolean) => {
      if (!shallLoad) return;
      this._configFacade.loadDataBuild(environment.data_build);
      numOfSecondaryConfigsToLoad++;
    };
    ...
    const checkConfigDep = (data: V2Config_MapDep | undefined) => {
      ...
      if (!data) {
        return of({ haltedStep: 'config' } as V1BaseAppInit_HaltedState).pipe(
          take(1),
        );
      }
          ...
          .pipe(
            ...
            exhaustMap((state) => {
              ...
              if (!state?.datas.getToken) {
                return of({
                  haltedStep: 'auth',
                } as V1BaseAppInit_HaltedState).pipe(take(1));
              }
  ...
  protected _setAuthConfig(baseUrl: string, environment: any) {
  ...
  protected _configDepLoaded(data: V2Config_MapDep) {
    ...
    this._editTitle(data.general.appName);
    this._addDepStyles();
