...
Sixteen rules. Every one but `sp-pin` guards a failure class that has actually happened here; `sp-pin` guards one this workspace created the conditions for the day it started pinning a version, and it is written the same way — as the thing no other signal can see. `--list` prints them; `--rule=<id>` runs one; `--json` for tooling.
