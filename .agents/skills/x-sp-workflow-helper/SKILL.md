...
| `scripts/messages.mjs` | **Nothing on its own** — every **verdict** the tool renders: each rule title, every failure, every skip. Each documented with where it is used, why it exists, and when a person sees it | one export per verdict; **telemetry** (counts, the run summary) stays with the code that formats it | **Read on every checker run.** **You edit it** when a verdict needs different words |
...
| `dead-messages`    | a verdict in the wrong place — exported but never wired (two copies that drift), or written inline instead of in the module. Checked both directions |
