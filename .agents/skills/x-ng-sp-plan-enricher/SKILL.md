...
- [ ] [enrich] 3. Global Constraints (CROSS-TASK only) — source-doc pointers, test-config, one-line restatements of the test/lib/divergence rules, commit pointer (merge, don't duplicate), plus the conditional ones: DEP JSON pair · companion scope guard · deferred chain
...
4. **Write each task's contract into the task itself** — not just its IDs (_Which channel carries what_, above). For every task that writes/updates tests **for this functionality**, the task text must carry all five of these. An ID list alone is what the baseline failed on.
   1. **Its exact IDs, and what each one becomes** — the FR/BR IDs that task's component(s) own (from this functionality's TFS ID Index / per-lib file), stated as the mapping and not just as a list: each **FR ID is a `describe` title**, each **BR ID is an `it` title**, IDs written exactly as the TFS has them. For an e2e task, the **AC IDs** it covers.
   ...
   **Why per task rather than once globally:** these five are the rules that decide whether the code an implementer writes is traceable, and the task's text is the only channel a script guarantees to deliver. Repeating a path in three tasks costs three lines; losing it costs the traceability the whole PRD/TFS system exists for.
...
- [ ] Global Constraints carries the cross-task set as one-liners: source-of-truth pointers, test-config, the commit-message pointer (`naming-conventions.md#git`), and one-line restatements of the test/lib/divergence rules — as pointers, never pasted copies.
...
- [ ] No workspace helper skill is named as something to *invoke*; the convention source-set defers to `AGENTS.md`'s hook. (A file-path pointer, like the CODEOWNERS item above, is not this — see Rules.)
...
| Hardcoding the list of helper skills to read                      | Defer to `AGENTS.md`'s Path A hook A1 `[gated]` band for the source-set; re-read from there if needed.                                                                          |
