...
This is not a style preference. **An implementer given FR/BR IDs but not their contract** uses the IDs as **code comments**, writes a `describe` that is not an FR, makes an FR ID a sibling `it`, adds tests that map to no requirement at all, and **silently reinterprets a BR** it finds ambiguous instead of reporting it. Every one of those is invisible afterwards: the tests pass, and the titles map to nothing.
...
6. **CODEOWNERS (when relevant)** — if the plan **creates** an owned path (a new app, lib, or shared version-folder) or the plan/user **explicitly** states an ownership handoff (named path + new owner), the create-path (or handoff) task must include a same-commit step pointing at `.agents/skills/x-codeowners-editor/SKILL.md` and `.agents/skills/x-codeowners-editor/scripts/upsert-owner.mjs` — do not paste the rulebook. **Never guess a handoff** from “we're changing this feature” or from file edits under an existing path. If neither condition holds, skip — do not add a standalone CODEOWNERS task.
...
- [ ] CODEOWNERS steps exist only when the plan creates owned paths or the plan/user explicitly states a handoff; each such step points at `.agents/skills/x-codeowners-editor/SKILL.md` and its script, not a pasted rulebook; no handoff was inferred; no standalone CODEOWNERS task otherwise.
...
| Pasting the whole CODEOWNERS rulebook into the plan               | Point at `.agents/skills/x-codeowners-editor/SKILL.md` and its script; that skill owns the rules.                                                                               |
