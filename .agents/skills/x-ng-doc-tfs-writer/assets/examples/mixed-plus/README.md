...
- **`[TO-CREATE]`** `shared-util-ng-money` → `formatAmount(minorUnits, currency)`, for the balance and transaction amounts. Does not exist yet. A `util` never gets `docs/x/` — its requirements live in its own `requirements/`, and creating it is a companion task in the plan. **Blocks:** `XWALLET-AC-01`.
