<!--
Template for `docs/x/{name}/TSD/page-v{n}.md` (a **shared** lib is versioned, so the version is in the filename; an **app-domain** lib is unversioned and keeps the plain name) — the `page` lib spec + its FR/BR.
