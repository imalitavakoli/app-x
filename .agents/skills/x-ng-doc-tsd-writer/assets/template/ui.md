<!--
Template for `docs/x/{name}/TSD/ui-v{n}.md` (a **shared** lib is versioned, so the version is in the filename; an **app-domain** lib is unversioned and keeps the plain name) — the `ui` lib spec + its FR/BR.
