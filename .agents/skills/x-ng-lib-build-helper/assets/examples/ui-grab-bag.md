...
> **A grab-bag is not a functionality.** It gets **no** `docs/x/{name}/` PRD or TFS, no ACs, and never e2e. Each item carries its own `requirements.md` for unit-test IDs. See `docs/getting-started/library-types-and-their-relationship.md` → Single-purpose vs grab-bag. Compare with [ui.md](ui.md), which is a **single-purpose** `ui` lib and *is* a functionality.
...
| Not here | Why |
| --- | --- |
| `docs/x/{name}/PRD.md` and `TFS/` | a grab-bag is not a functionality |
