...
The item's own FR/BR registry — this is what its unit tests map to, because a grab-bag has no TSD. IDs are `UI-{KEY}-…`, where `{KEY}` is the folder basename minus `-v{n}`, uppercased (`truncate-v1` → `TRUNCATE`):
