...
- Depends on a shared avatar/image UI lib to render the avatar. **It must change for us**: it needs an `icoPlaceholder` input so a user with no `avatarUrl` still renders something. That change is specified in that lib's own docs, not here. Blocks `AVATAR-AC-04`.
- Risk: until that lib gains its `icoPlaceholder` input, a user with no `avatarUrl` renders an empty avatar slot.
