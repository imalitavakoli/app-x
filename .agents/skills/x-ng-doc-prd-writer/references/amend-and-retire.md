...
3. **Its TSD FR/BRs are now suspect.** Every FR/BR that back-links this AC was written against the old wording. Say so; correcting them is an `x-ng-doc-tsd-writer` run, not something to patch from here.
...
3. **Report the orphans it leaves** — every TSD FR/BR that back-linked it, and any e2e `it` titled with it. Those must be retired or re-pointed in their own runs; this skill does not touch them.
...
| Retiring an AC and leaving its FR/BRs back-linking it | Report every orphan; they are corrected in their own TSD run |
