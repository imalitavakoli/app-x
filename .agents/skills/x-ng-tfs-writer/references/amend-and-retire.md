...
The test for amend vs retire+add: *is this the same rule with a corrected description, or a different rule?* A threshold boundary flipping from exclusive to inclusive is the **same** rule stated wrongly → amend. Replacing "retry button" with "auto-retry after 5s" is a **different** rule → retire the old, add a new one.
...
1. **Remove the entry** from its `{libtype}.md`.
2. **Remove its ID Index row** from `README.md`. An ID Index row for a deleted entry is the most misleading state possible — it advertises coverage that does not exist.
...
| Renumbering to close the gap a retirement leaves | Never — the gap is the point; a burned number must stay burned |
| Deleting the entry but leaving its ID Index row | The row advertises coverage that no longer exists — remove both |
