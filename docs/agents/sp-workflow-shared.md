...
2. **Spec & plan location — Superpowers defaults, tracked.** Do **not** override where `brainstorming` and `writing-plans` write. Specs land at `docs/superpowers/specs/` and plans at `docs/superpowers/plans/`. Follow Superpowers' own commit of the spec. The plan is tracked; do not skip `git add` as if the path were ignored. `/.superpowers/` stays git-ignored for Superpowers' own scratch (SDD briefs, brainstorm companion) — that directory is **not** where specs or plans live. The durable product docs remain the PRD and TSD under `docs/x/{domain}/{name}/` (Path A's PRD/TSD-over-cycle-spec rule).
...
**A functionality's docs are the user's commit, on request.** `docs/x/{domain}/{name}/PRD/` and `TSD/` are written before any feature branch exists, so no row above covers them and no hook commits them. The close-out that ends documentation **asks** the user to commit them once both writers are done — and names this cycle's Superpowers spec and plan under `docs/superpowers/` when those files have uncommitted changes (the spec may already have been committed by `brainstorming`; the plan, and any later spec sync, often have not). If they decline, the files stay uncommitted and that is reported plainly, never worked around by committing them anyway.
