...
When we already have our apps and libs, we can generate stuff inside them!
...
#### Use `@nx/angular` plugin to generate stuff that the plugin's generator supports:

**Note!** By running `nx g`, you are actually running the generator command of NX itself. e.g., `@nx/angular` specifies the name of the plugin installed in the workspace, and `:component` specifies the name of the generator.

- Run `nx g @nx/angular:component libs/collective/type/project-name/src/lib/component-name` to generate a **component**.

- Run `nx g @nx/angular:directive libs/collective/type/project-name/src/lib/directive-name` to generate a **directive**.

- Run `nx g @nx/angular:pipe libs/collective/type/project-name/src/lib/pipe-name` to generate a **pipe**.

**Tip!** [Click here](https://nx.dev/docs/technologies/angular/generators) to read the latest documentation about `@nx/angular` plugin (that is installed in NX) and its generators.

&nbsp;

#### Use `@schematics/angular` to generate stuff that `@nx/angular` plugin's generator doesn't support:

**Note!** If you try to invoke a generator that is not present in `@nx/angular` plugin, the request will automatically be forwarded to `@schematics/angular`.
...
**Tip!** [Click here](https://angular.dev/cli/generate) to read the latest documentation about Angular's generators.
...
- _Optional!_ Run `nx g @nx/angular:component {my-component} --dry-run` to just see the command's reports activity without running the command for real! i.e., by adding `--dry-run` flag to our command, we can see what files the command is going to change in our project, whenever we call it without the flag.

- **Generating a lib schema**: Run `nx g @nx/angular:lib --name=collective-type-name --directory=libs/collective/type/name` to generate an Angular lib in the `libs` folder of the workspace.  
