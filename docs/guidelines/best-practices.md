...
- **Prioritize docs**. Before generating/updating a project (app or lib), always begin by writing or updating its `README.md` (for apps, it's also essential to update the `CHANGELOG.md` file).  
  **Note!** Keep the root `CODEOWNERS` file current when you add or hand off an app/lib — follow the rules at the top of that file. This helps the Product Manager (PM) assign upgrade/bug-fix work to the right owner.
