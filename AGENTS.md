...
1. **Isolation — work in place, no worktree.** `using-git-worktrees` honours a declared preference without asking: in this workspace do **not** create a worktree — create the feature branch in the existing checkout (the repo directory you are already in) and work there. A branch already keeps the trunk safe; a worktree would additionally cost a full dependency install and a cold Nx cache every cycle, and `subagent-driven-development` gains nothing from one — it dispatches implementer subagents **one at a time** (running them in parallel is on that skill's "Never" list), so its isolation is of _context_, not of the filesystem.
   ...
   > **Note! — if we ever want worktrees instead.** Change **only** this preference: drop the "do not create a worktree" instruction, and `using-git-worktrees` goes back to its own default — it asks for consent and creates one. Nothing else in this file moves: the hook tables, their order, and both execution modes stay exactly as they are.
   >
   > Two reasons we might: **(a)** the cost we're avoiding is a full dependency install plus a cold Nx cache in every new worktree — if this harness's built-in worktree tool provisions dependencies cheaply, most of that cost disappears (worth testing once on a throwaway branch); **(b)** a worktree is the only way to run two feature cycles at the same time, or to let an agent build while you keep using your own checkout.
   >
   > If we do switch, revisit one thing: a git-ignored folder **inside** a worktree is deleted when finishing cleans that worktree up — so `.superpowers/` (the spec, plan and SDD ledger) would need to live outside it to survive the cycle.
