...
| Before `writing-plans`                                                                       | Write/refresh the PRD & TFS (`x-ng-prd-writer`, then `x-ng-tfs-writer`), then load the reference guidelines (`x-ng-lib-build-helper`, `x-ng-test-unit-helper`, `x-ng-test-e2e-helper`) — so all of it is in context and `writing-plans` drafts the plan from it. If `x-ng-tfs-writer` flags a product-observable gap (a `(new — suggest a PRD AC)` entry), ask the user; if approved, re-run `x-ng-prd-writer` to add the AC, then re-run `x-ng-tfs-writer` to back-link it. |
| After `writing-plans`, before execution                                                      | `x-ng-sp-plan-enricher` — fold the PRD/TFS IDs and rules (plus the commit-message pointer, since subagents commit but don't read this file) into the plan's Global Constraints (so they reach execution subagents) and tag its test tasks.                                                                                                                                                                                                                                   |
| `using-git-worktrees` (worktree/branch created at execution start)                           | — (no step needed: the agent creates the branch itself, and AGENTS.md already tells it to name branches per `naming-conventions.md#git`)                                                                                                                                                                                                                                                                                                                                     |
